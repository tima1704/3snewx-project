...
interface AddEditOfferKPITelplateProps {
  editData?: IKPITemplate;
  setEditData: (value: SetStateAction<IKPITemplate | undefined>) => void;
}

export default function AddEditOfferKPITelplate({
  editData,
  setEditData,
}: AddEditOfferKPITelplateProps) {
  const [onFocus, SetFocus] = useState(false);
  const { languagesNotGeneral } = useLanguage();

  const onChange = (
    e: React.ChangeEvent<HTMLInputElement>,
    key?: keyof IKPITemplate
  ) => {
    if (editData) {
      setEditData({
        ...editData,
  ...
  const { execute, disabled, errors } = useAsyncSave(
    SettingsApi.saveSettingsOfferKpiTemplate,
    ...
    <Collapse in={editData ? true : false}>
      <div className={"spaceBlock32"}>
        <SelectLang
          ...
          onSelect={() => {}}
        />
        <ErrorInputMessage errors={errors} inputName={"offer_kpi_template"} />
        <InputComp
          errors={errors}
          name={"title"}
